...
class DataLoader:
    def __init__(self, config_id, config_sheet_name):
        try:
            self.gc = gspread.service_account(filename='backlog_credentials/credentials.json')
    ...
    def get_backlog_const(self):
        backlog_row_cell = self.config_page.find("backlog_row")
        backlog_row = self.config_page.cell(backlog_row_cell.row, backlog_row_cell.col + 1).value
        if backlog_row != None:
            return int(backlog_row)
        else:
            print('error in backlog row')

    def update_backlog_row(self, new_value):
        config_id, config_sheet_name = self.conf_sheet()
    ...
    def read_backlog_scan_ids(self) -> str:
        backlog_scan_id, backlog_scan_sheet_name = self.retry_with_backoff(lambda: self.backlog_scan_sheet())
        sh = self.retry_with_backoff(lambda: self.gc.open_by_key(backlog_scan_id))
        page = self.retry_with_backoff(lambda: sh.worksheet(backlog_scan_sheet_name))
        ids_col_num = 1
        backlog_row = self.retry_with_backoff(lambda:self.get_backlog_const())
        while True:
            idi_cell = self.retry_with_backoff(lambda: page.cell(backlog_row, ids_col_num))
            ...
            if idi is not None:
                ...
                time.sleep(86400)
                print('Ошибочка чтения read_periodic_scan_ids')

        self.update_backlog_row(backlog_row)
        return idi

    def write_backlog_scan_data(self, info: ChannelDetails):
        backlog_scan_id, backlog_scan_sheet_name = self.retry_with_backoff(lambda:self.backlog_scan_sheet())
        sh = self.retry_with_backoff(lambda: self.gc.open_by_key(backlog_scan_id))
        page = self.retry_with_backoff(lambda: sh.worksheet(backlog_scan_sheet_name))

        backlog_row = self.retry_with_backoff(lambda:self.get_backlog_const() - 1)
        ...
        for attribute, col_num in column_mapping.items():
            ...
            self.retry_with_backoff(lambda: page.update_cell(backlog_row, col_num, cell_value))
